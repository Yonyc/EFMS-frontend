...
export default function ParcelsPage() {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        setError(null);
        apiGet(`/farm/${selectedFarm.id}/parcels`)
            .then(r => r.json())
            .then(data => { if (active) setParcels(data); })
            .catch(() => { if (active) setError(t('parcels.loadError', { defaultValue: 'Failed to load parcels' })); })
            .finally(() => { if (active) setLoading(false); });
        return () => { active = false; };
    }, [selectedFarm?.id, t, refreshNonce]);
    ...
    const filtered = useMemo(() => parcels.filter(p => {
        ...
        return [p.name, p.sourceName, p.sourceCode, pp?.cultureLabel ?? null, p.municipality, p.exploitantName]
            .some(v => v?.toLowerCase().includes(q));
    }), [parcels, rowPeriods, search]);
        ...
        <ProtectedRoute>
            <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 text-slate-50">
                <div className="mx-auto max-w-7xl px-4 py-8">
                    ...
                    {!loading && selectedFarm && filtered.length === 0 && !error && (
                        <div className="flex items-center justify-center py-20 text-slate-400">
                            {search
                    ...
                    {!loading && filtered.length > 0 && (
                        <div className="overflow-x-auto rounded-2xl border border-white/10 bg-white/5 shadow shadow-black/20">
                            ...
                            <div className="border-t border-white/5 px-4 py-2 text-xs text-slate-500">
                                {t('parcels.total', { count: filtered.length, defaultValue: '{{count}} parcel(s)' })}
                            </div>
                        </div>
                    )}
                </div>
