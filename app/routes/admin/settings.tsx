    ...
    return (
        <div className="space-y-6 max-w-3xl">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <h2 className="text-2xl font-extrabold text-slate-900">{t('admin.settings.title', { defaultValue: 'Global Settings' })}</h2>
            </div>

            {loading ? (
                <div className="text-slate-500 p-8 text-center bg-white border border-slate-200 rounded-2xl">
                    {t('admin.settings.loading', { defaultValue: 'Loading settings...' })}
                </div>
            ) : (
                <div className="space-y-6">
                    <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-xl shadow-slate-900/10">
                        ...
                        <form onSubmit={handleSave} className="space-y-8">
                            <div className="space-y-4">
                                <label className="flex items-start gap-4 p-4 rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-50 transition-colors cursor-pointer group">
                                    <div className="pt-1">
                                        <input 
                                            type="checkbox" 
                                            checked={settings.userVerificationRequired || false}
                                            onChange={(e) => setSettings({ ...settings, userVerificationRequired: e.target.checked })}
                                            className="w-5 h-5 rounded border-slate-300 bg-white text-indigo-500 focus:ring-indigo-500 focus:ring-offset-white transition-all cursor-pointer" 
                                        />
                                    </div>
                                    <div className="flex flex-col">
                                        <span className="text-sm font-bold text-slate-700 group-hover:text-indigo-300 transition-colors">
