    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <h2 className="text-2xl font-extrabold text-slate-900">{t('admin.users.title', { defaultValue: 'Users Management' })}</h2>
            </div>
            ...
            <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-xl shadow-slate-900/10 space-y-4 sm:space-y-0 sm:flex sm:gap-4 sm:items-end">
                <div className="flex-1 space-y-2">
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('admin.users.searchLabel', { defaultValue: 'Search' })}</label>
                ...
                <div className="space-y-2">
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('admin.users.verificationLabel', { defaultValue: 'Verification' })}</label>
                    <select 
                ...
                <div className="space-y-2">
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('admin.users.roleLabel', { defaultValue: 'Role' })}</label>
                    <select 
                        value={filterAdmin} 
            ...
            <div className="bg-white border border-slate-200 rounded-2xl shadow-xl shadow-slate-900/10 overflow-hidden">
                {loading ? (
                    <div className="p-8 text-center text-slate-500">{t('admin.users.loading', { defaultValue: 'Loading users...' })}</div>
                ) : filteredUsers.length === 0 ? (
                    <div className="p-8 text-center text-slate-500">{t('admin.users.noUsers', { defaultValue: 'No users found matching your filters.' })}</div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-slate-200 bg-slate-50">
                                    <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('admin.users.tableUser', { defaultValue: 'User' })}</th>
                                    <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('admin.users.tableStatus', { defaultValue: 'Status' })}</th>
                                    <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('admin.users.tableRole', { defaultValue: 'Role' })}</th>
                                    <th className="px-6 py-4 text-right text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('admin.users.tableActions', { defaultValue: 'Actions' })}</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <button onClick={() => toggleUserAdmin(u.id, u.admin)}
                                                className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold transition-all ${u.admin ? 'bg-purple-500/10 text-purple-400 border border-purple-500/20 hover:bg-purple-500/20' : 'bg-slate-100 text-slate-500 border border-slate-300 hover:bg-slate-100'}`}>
            ...
            {editingUser && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                    <div className="bg-white border border-slate-200 rounded-2xl p-6 w-full max-w-md shadow-2xl">
                        <h3 className="text-xl font-bold mb-6 text-slate-900">{t('admin.users.editTitle', { defaultValue: 'Edit User' })}</h3>
                        <form onSubmit={submitEditUser} className="space-y-4">
                            <div className="space-y-1.5">
                                <label className="block text-sm font-medium text-slate-600">{t('admin.users.usernameLabel', { defaultValue: 'Username' })}</label>
                            ...
                            <div className="space-y-1.5">
                                <label className="block text-sm font-medium text-slate-600">{t('admin.users.emailLabel', { defaultValue: 'Email' })}</label>
                            ...
                            <div className="flex justify-end gap-3 pt-6 mt-6 border-t border-slate-200">
                                <button type="button" onClick={() => setEditingUser(null)}
                                    className="px-4 py-2 text-sm font-semibold text-slate-600 bg-slate-100 rounded-xl hover:bg-slate-100 transition-colors">
